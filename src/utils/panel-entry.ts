...
const saveStatus = (status: PanelEntryStatus | null) => {
  if (!status) return;
  useConfigStore().setPanelEntryAccess({
...
export const getPanelEntryStatus = async (force = false) => {
  ...
  statusPromise = (async () => {
    try {
      const response = await window.fetch("/v1/panel-entry/status", {
        credentials: "include",
        headers: {
      ...
      const cached = useConfigStore().panelEntryAccess;
      if (cached) {
        return {
...
export const isPanelEntryPathAllowed = (
  status: PanelEntryStatus | null,

...
const conf = reactive({
  ...
  rules: {
    username: [
      { required: true, message: t("commons.rule.username"), trigger: "blur" },
    ],
    password: [
      { required: true, message: t("commons.rule.password"), trigger: "blur" },
    ],
  } as FormRules,
  handleLogin: () => {
...
<template>
  <login-container :loading="conf.loading" currentActive="login">
    <template #default="{ className }">
      <div
        class="logo-box flex justify-center fit-width"
      ...
      <div class="" :class="{ 'login-title-box': isMobile }">
        ...
        >
          <el-form-item
            :class="[className.formItemGap, { 'input-box': !isMobile }]"
            prop="username"
          >
            <el-input
              v-model.trim="conf.form.username"
              :placeholder="$t('commons.login.usernamePlaceholder')"
              clearable
          ...
          <el-form-item prop="password" :class="{ 'input-box': !isMobile }">
            <el-input
              v-model.trim="conf.form.password"
              :placeholder="$t('commons.login.passwordPlaceholder')"
              type="password"
              clearable
              :class="{ 'pc-input': isMobile }"
            >
          ...
          <div class="flex justify-between">
            <el-checkbox
              ...
              :label="$t('commons.login.remeber')"
            />
          </div>
          <button
            :class="[className.loginBtn, { 'mobile-btn': isMobile }]"
            @click.prevent="conf.handleLogin"
          >

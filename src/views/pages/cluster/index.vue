...
const dispatch = reactive({ websiteId: 0, strategy: 'least_load', nodeIds: [] as number[], tags: '' })
...
const dispatchWebsite = async () => {
  ...
  try {
    const result = await Api.dispatchWebsiteToCluster({ websiteId: dispatch.websiteId, strategy: dispatch.strategy, nodeIds: dispatch.nodeIds, tags: dispatch.tags.split(',').map((item) => item.trim()).filter(Boolean), idempotencyKey: `website-${dispatch.websiteId}-${Date.now()}` })

...
const handleKeydown = (e: KeyboardEvent | Event) => {
  ...
  if (event.key === 'Enter') {
    ...
    // 找到光标所在的行和位置
    const currentLineNumber = content.substr(0, cursorPos).split('\n').length - 1
    const currentLine = lines[currentLineNumber] || ''
    const cursorPosInLine = cursorPos - content.split('\n').slice(0, currentLineNumber).join('\n').length - 1
    ...
    const newLines = lines.map((line, index) => {
      ...
      if (trimmedLine || index === currentLineNumber + 1) {  // 修改这里，确保新行也会添加序号
    ...
    memo.content = newLines.join('\n')
    
    // 设置光标位置
    nextTick(() => {
      const newPos = newLines.slice(0, currentLineNumber + 1).join('\n').length + `\n${currentLineNumber + 2}. `.length
...
onMounted(() => {
  if (memo.content && !memo.content.startsWith('1.')) {
    memo.content = `1. ${memo.content}`
  }
})
</script>
...
<template>
  <custom-dialog :show="show" title="新建备忘录" width="624px" :show-close="false" @update:show="close">
    <el-input v-model="memo.content" type="textarea" :rows="20" placeholder="请输入备忘录内容" @keydown="handleKeydown" />
    <template #footer>

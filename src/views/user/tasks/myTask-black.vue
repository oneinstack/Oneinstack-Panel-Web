<template>
  <x-page :no-footer="true" :fixed="false">
    ...
    <template #top>
      <div class="type-nav">
        <custNav :dataArr="conf.typeList" @change="(e) => {conf.changeType(e.index, e.item)}" />
      </div>
    </template>
    <div class="task-list" style="flex: 1; overflow-y: hidden" v-if="conf.defalutWallet">
      <div style="height: 100%; overflow-y: scroll" @scroll="conf.moreMessage">
        <template v-for="(item, index) in conf.taskList" :key="index">
          <div class="item-head flex-b-c">
            <span>{{ conf.typeList.slice(1).find(it => it.id == item.userTaskStatus)?.name }}</span>
            <span>{{ sutil.getTimeFormat(item.pickupTime) }}</span>
          </div>
          <div class="task-item">
            <div class="task-left">
              <div class="left-img">
            ...
            <div class="task-right">
              <div class="right-line">
                <div :class="'get-line-' + item.userTaskStatus" :style="{ 'width': conf.getProgress(item) + '%' }">
                </div>
              </div>
        ...
        <noData v-if="!conf.loading && conf.taskList.length == 0"></noData>
      </div>
    </div>
  </x-page>
</template>
...
import noData from '../bonus/com/noData.vue'

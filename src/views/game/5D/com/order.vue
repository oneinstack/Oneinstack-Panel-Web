<template>
	<div class="result">
		<div class="result-list">
				...
				<div class="result-ltem" :id="index == 0 ? 'order0' : ''">
					<div class="result-title">
						...
						<div class="title-right">
							<div class="purchase-bg bet-type">{{ item.betNumber[item.betNumber.length - 2] }}</div>
							<div class="purchase-bg" v-if="item.betNumber[item.betNumber.length - 1] == 'odd'"
								style="background: linear-gradient(90deg, #fd0261 0%, #ff8a96 100%);">
								{{ item.betNumber[item.betNumber.length - 1] }}
							</div>
							<div class="purchase-bg" v-if="item.betNumber[item.betNumber.length - 1] == 'even'"
								style="background: linear-gradient(90deg, #00be50 0%, #9bdf00 100%);">
								{{ item.betNumber[item.betNumber.length - 1] }}
							</div>
							<div class="purchase-bg" v-if="item.betNumber[item.betNumber.length - 1] == 'small'"
								style="background: linear-gradient(90deg, #ff9000 0%, #ffd000 100%);">
								{{ item.betNumber[item.betNumber.length - 1] }}
							</div>
							<div class="purchase-bg" v-if="item.betNumber[item.betNumber.length - 1] == 'big'"
								style="background: linear-gradient(90deg, #00bdff 0%, #5bcdff 100%);">
								{{ item.betNumber[item.betNumber.length - 1] }}
							</div>
							<div class="purchase-num"
								v-if="item.betNumber[item.betNumber.length - 1] >= 0 && item.betNumber[item.betNumber.length - 1] < 10">
								<div class="num">
									<img src="/static/img/poinits.webp" />
									<span>{{ item.betNumber[item.betNumber.length - 1] }}</span>
								</div>
							</div>
						</div>
					</div>
							...
							<div class="line-left">
								<div class="content-item">
									<div class="content-img-item">
										...
										<div class="img-num">
											<span style="margin-top: 0rem;">{{ item.openNumberArr[0] || '-' }}</span>
										</div>
									</div>
									<div class="content">
										{{ $t('game.openCode') }}:
										<div class="banner-item" v-if="item.openNumberArr">
											<div class="num" v-for="(into, intoIndex) in item.openNumberArr"
												:key="intoIndex">
												<img src="/static/img/poinits.webp" />
												<span>{{ into }}</span>
											</div>
							...
							<div class="line-right">
								<div class="content-item">
									<div class="content-img-item">
										<img src="/static/img/bet-numbers.png" />
										<div class="img-num">
											<span style="margin-top: 0rem;">{{ 'S' }}</span>
										</div>
									</div>
									<div class="content">
										{{ $t('game.sum') }}:
										<div class="banner-item" v-if="item.sum">
											<div class="num">
												<img src="/static/img/poinits.webp" />
												<span>{{ item.sum }}</span>
											</div>
			...
			<no-data v-if="orderDataList.length == 0"></no-data>
		</div>
	</div>
</template>

<template>
  <gameBox
    ...
    @change="conf.play.choose"
    @close="conf.bet.closeFun"
    :selectBetInfo="conf.selectBetInfoArr"
    :gameType="'5D'"
    :currentOpenInfo="conf.currentOpenInfo"
  ...
  >
    ...
    <template v-slot:bet>
      <div class="row items-center">
        ...
        <template v-for="(item, itemIndex) in conf.selectBetInfoArr" :key="itemIndex">
          ...
          <div class="bet-number flex-center" :class="['num_' + item.color]" v-else>{{ item.number }}</div>
        </template>
      </div>
    </template>
    <!-- 开奖结果 -->
    <div class="game-result">
      <div class="result-top">
        ...
        <div class="round-num">
          <!-- 已开奖 -->
          <div
            class="fade-item fage-active"
            :class="conf.resultInfoObj.sum > 22 ? 'num_blue' : 'num_yellow'"
            v-if="!conf.isNoOpenCode"
          >
            {{ conf.resultInfoObj.sum }}
          </div>
          <!-- 未开奖 -->
          <div class="fade-item fage-active" :class="conf.resultInfoObj.sum > 22 ? 'num_blue' : 'num_yellow'" v-else>
            {{ conf.scollSum }}
          </div>
        </div>
      </div>
    ...
    <div class="bet-select-box">
      ...
      <div class="bet-type-list">
        <template v-for="(item, index) in conf.selectBetList" :key="index">
          <div
            class="d5-bet-type flex-center"
            :class="{ 'type-active': item == conf.selectBetType }"
            @click="conf.handleSelectBetType(item, index)"
          >
            {{ item }}
          </div>
        </template>
      ...
      <div class="d5-bet-content">
        <div class="flex-bw">
          <template v-for="(item, index) in conf.betTypeOdds" :key="item.name">
            <div class="bet-type-btn flex-bw" :class="['num_' + item.color]" @click="conf.handleSelectBet(item)">
              <span>{{ item.number }}</span>
              <span>{{ item.odds }}</span>
            </div>
          </template>
    ...
    <div class="d5-list">
      <div class="d5-list-type flex-bw">
        <div
          ...
          :class="{ 'type-active': conf.tabType == 1 }"
          @click="conf.handleSelctTabChange(1, null)"
        >
          {{ $t('game.resultHistory') }}
        </div>
        <div
          class="type-item flex-center"
          :class="{ 'type-active': conf.tabType == 2 }"
          @click="conf.handleSelctTabChange(2, null)"
        >
          {{ $t('winGo.Chart') }}
        </div>
        <div
          class="type-item flex-center"
          :class="{ 'type-active': conf.tabType == 4 }"
          @click="conf.handleSelctTabChange(4, null)"
        >
          {{ $t('game.rule') }}
        </div>
        <div
          class="type-item flex-center"
          :class="{ 'type-active': conf.tabType == 3 }"
          @click="conf.handleSelctTabChange(3, null)"
        >
          {{ $t('game.myOrder') }}
        </div>
      </div>
      ...
      <order
        v-if="conf.tabType == 3"
        :orderDataList="conf.orderDataList"
    ...
    <!-- 分页工具 -->
    <cpage
      :pageInfo="conf.pageObj"
      style="margin-top: 30rem"
      v-if="
        conf.tabType == 1 || (conf.tabType == 2 && conf.selectBetIndex != 5)
          ? conf.resultList.length > 0
          : conf.tabType == 3
            ? conf.orderDataList.length > 0
            : ''
      "
